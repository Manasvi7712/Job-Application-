...
3. **A title appearing in search results does not mean the specific posting is currently open — watch for staleness signals and downgrade when you see them.** Confirmed three times (Cummins Atlanta 8/23 was Senior not entry, Flexport 9/1 actually required 3-6 yrs not entry-level, Brex 9/3 showed "no longer accepting applications"/"removed" dates from a full year earlier — almost certainly a stale prior cohort of a recurring annual program). When a secondary job board shows a removal/closed date, or a role's real requirements contradict what was previously assumed from the title alone, say so explicitly in that day's log and downgrade the listing's shortlist rank — don't let an outdated assumption silently persist across days. Recurring rotational/cohort programs (Brex, Freddie Mac, JPMorgan, etc.) are especially prone to this since last year's listing and this year's can look identical in a title-only search.
